...
public interface QueueJpaRepository  extends JpaRepository<Queue, Long> {
     ...
     */
    Optional<Queue> findFirstByStatusOrderByEnteredAtDesc(TokenStatus status);
     ...
     */
    @Query("SELECT t FROM Queue t WHERE t.status = :status AND t.enteredAt < :time")
    List<Queue> findActiveTokensEnteredBefore(@Param("status") TokenStatus status, @Param("time") LocalDateTime time);
     ...
     */
    @Query("SELECT t FROM Queue t WHERE t.status = :status ORDER BY t.createdAt ASC")
